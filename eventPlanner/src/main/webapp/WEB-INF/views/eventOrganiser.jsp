...
<title>Event planner - planning</title>
						...
						<c:choose>
							<c:when test="${not empty eventOrganiser.associatedEvents}">
								<ul class="list-unstyled">
									<c:forEach items="${eventOrganiser.associatedEvents}"
										var="event">
										<c:if test="${event.eventStatus == 'Initiated' }">
											<a
												href="<c:url value='/organiser${eventOrganiser.eventOrganiserId}/plan/event${event.eventId}' />">
												<li
												class="list-group-item list-group-item-action flex-column align-items-start">
													<h5 class="mb-1">${event.eventName}
														<small> on <joda:format
																value="${event.eventPlannedDate}" pattern="dd-MM-yyyy" />
														</small>
													</h5>
													<p>About: ${event.eventDescription}</p>
													<p>${fn:length(event.associatedTasks)}&nbsp;tasks&nbsp;and&nbsp;${fn:length(event.associatedActivities)}&nbsp;activites</p>
											</li>
											</a>
										</c:if>
									</c:forEach>
